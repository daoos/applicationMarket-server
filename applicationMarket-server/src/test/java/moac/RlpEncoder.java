 ...
 * yellow paper</a> and <a href="https://github.com/moac/wiki/wiki/RLP">here</a>.</p>
 */
public class RlpEncoder {
    ...
    static byte[] encodeString(RlpString value) {
        return encode(value.getBytes(), RlpDecoder.OFFSET_SHORT_STRING);
    }
    ...
    static byte[] encodeList(RlpList value) {
        ...
        if (values.isEmpty()) {
            return encode(new byte[]{ }, RlpDecoder.OFFSET_SHORT_LIST);
        } else {
            byte[] result = new byte[0];
            for (RlpType entry:values) {
                result = concat(result, encode(entry));
            }
            return encode(result, RlpDecoder.OFFSET_SHORT_LIST);
        }
    }
